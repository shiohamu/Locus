...
export async function editNote(page: Page, noteId: string, newTitle: string, newContent: string) {
  ...
  // "保存中..." が消えるのを待つ
  await page
    .waitForSelector('button:has-text("保存中...")', { state: "hidden", timeout: 10000 })
    .catch(() => {});
...
export async function addTagToNote(page: Page, noteId: string, tagName: string) {
  ...
  const addTagButton = page
    .locator('button:has-text("+ タグを追加"), button:has-text("タグを追加")')
    .first();
...
export async function searchNotes(page: Page, query: string) {
  ...
  const searchInput = page
    .locator(
      'input[type="search"], input[placeholder*="検索"], input.search-input, input[type="text"]'
    )
    .first();
  ...
  await page
    .waitForSelector('button:has-text("検索中...")', { state: "hidden", timeout: 15000 })
    .catch(() => {});
...
export async function createRSSFeed(page: Page, url: string, title: string) {
  ...
  // 新規登録フォームを表示
  const newFeedButton = page
    .locator('button:has-text("新規登録"), button:has-text("追加")')
    .first();
