...
import { createRSSFeed, goToRSS } from "./helpers.js";
...
test.describe("RSS機能", () => {
  ...
  test("RSSフィードを登録できる", async ({ page }) => {
    ...
    const newFeedButton = page
      .locator(
        'button:has-text("フィードを追加"), button:has-text("新規登録"), button:has-text("追加")'
      )
      .first();
    ...
    if ((await urlInput.isVisible()) && (await titleInput.isVisible())) {
      ...
      await page
        .waitForSelector('button:has-text("登録中...")', { state: "hidden", timeout: 10000 })
        .catch(() => {});
      // フィード一覧が更新されるまで待つ
